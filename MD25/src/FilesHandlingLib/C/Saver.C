...
bool Saver::appendToFile(const std::string& text) const {
  std::ofstream fout{ filePath_, std::ofstream::app };
  if (!fout.is_open())
    return false;
...
void Saver::appendChar(std::ofstream& fout, const char& chr) const {
  fout.put(chr);
}

void Saver::appendText(std::ofstream& fout, const std::string& text) const {
