...
void Decryptor::decrypt(const std::string& encryptedTxt) {
  for (size_t i{ 0 }; i < encryptedTxt.size(); i += 4) {
    char encr1{ encryptedTxt[i] }, encr2{ encryptedTxt[i + 1] }, 
      encr3{ encryptedTxt[i + 2] }, encr4{ encryptedTxt[i + 3] };

    char decr1{ (encr1 & 0b1000000) << 1 | encr4 & 0b1000000 | (encr3 & 0b10000) << 1 | (encr2 & 0b1000) << 1 | 
      (encr1 & 0b100) << 1 | encr4 & 0b100 | encr3 & 0b10 | encr2 & 0b1 };
    char decr2{ (encr2 & 0b1000000) << 1 | (encr1 & 0b10000) << 2 | (encr4 & 0b10000) << 1 | (encr3 & 0b1000) << 1 |
      (encr2 & 0b100) << 1 | (encr1 & 0b10) << 1 | encr4 & 0b10 | encr3 & 0b1 };
    char decr3{ (encr3 & 0b1000000) << 1 | (encr2 & 0b10000) << 2 | (encr1 & 0b1000) << 2 | (encr4 & 0b1000) << 1 |
